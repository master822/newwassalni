...
router.post('/users/:id/adjust-wallet', async (req, res) => {
    ...
    try {
        ...
        const adjustmentReason = reason || 'تعديل رصيد إداري';
       ...
       VALUES ($1, $2, 'ADMIN_ADJUSTMENT', $3, $4, $5, 'COMPLETED')`,
            [
                ...
                `تعديل رصيد إداري (${deltaPoints > 0 ? '+' : ''}${deltaPoints} نقطة): ${adjustmentReason.trim()}`,
            ]
        );
       ...
       VALUES ($1, $2, 'تحديث رصيد المحفظة', $3, 'SYSTEM')`,
            [
                uuidv4(),
                id,
                `تم ${deltaPoints > 0 ? 'إضافة' : 'خصم'} ${Math.abs(deltaPoints)} نقطة من محفظتك. السبب: ${adjustmentReason.trim()}. الرصيد الحالي: ${newBalance} نقطة.`,
            ]
        );
        ...
        await logAdminActivity(
            ...
            req.user.email,
            'تعديل رصيد مالي',
            `تعديل رصيد ${userRes.rows[0].name} من ${oldBalance} إلى ${newBalance} (${deltaPoints > 0 ? '+' : ''}${deltaPoints} نقطة). السبب: ${adjustmentReason.trim()}`,
            id,
            req.ip
        );
